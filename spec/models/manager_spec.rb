...
describe 'Manager' do
  ...
  it 'has employees' do
    expect(@manager.employees.size).to eq(1)
    expect(@manager.employees.first.name).to eq('brandon')
  end

  it 'has forms' do
    expect(@manager.forms.size).to eq(1)
    expect(@manager.forms.first.title).to eq('Bathroom')
  end

  it 'has completed_forms' do
    expect(@manager.completed_forms.size).to eq(1)
  end
end
